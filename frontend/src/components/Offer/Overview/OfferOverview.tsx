        ...
        <div>
            <div className=" h-auto bg-gray-300 rounded-lg mx-2 sm:mx-6 my-4 shadow-lg shadow-gray-300 relative">
                <div className="max-w-screen-xl px-4 py-3 mx-auto">
                    <div
                        className="flex flex-col sm:flex-row font-medium mt-0 space-y-2 sm:space-x-2 sm:space-y-0 text-sm whitespace-nowrap sm:items-center">
                        <div>
                        ...
                        {(offers && offers.length === 0) ? <span className="mt-4 mx-10 bg-red-400 rounded-lg px-2 py-2 shadow-lg shadow-gray-300">No offers.</span> : <><div className="flex flex-col">
                            <button
                                type="button"
                ...
                <div>
                    <button type="button" onClick={() => refetch()}
                            className="text-black hover:underline">Refresh
                    </button>
                </div>
                        </>}
                    </div>
                    ...
                    <ul>
                        {(offers && users) ?
                            filteredOffers.map((offer) =>
                                <OfferOverviewItem key={offer.id} offer={offer}
                                                   seller={users?.find(u => u.id === offer.userId) ?? users[0]}
                                                   item={getItemByIdFromQuery(items, offer.itemId)}/>) :
                            <span>Loading...</span>}
                    </ul>
                </section>
            </div>
